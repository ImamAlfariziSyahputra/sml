...
import yesnoOptions from '../../../../data/pelanggan/yesno.json';
...
import periodePenagihanOptions from '../../../../data/pelanggan/periodePenagihan.json';
// Actions
...
import {
  ...
  updatePelanggan,
  resetPelanggan
} from '../../../../redux/actions/pelangganAction';
...
export default function AddPelanggan() {
  ...
  const { pelanggan, loading, error } = useSelector((state) => state.pelanggan);
  ...
  const onSubmit = (data) => {
    ...
    return dispatch(updatePelanggan(data, id))
      .then(() => {
  ...
  useEffect(() => {
    ...
    dispatch(getPelanggan(id));
    dispatch(getCabangs());
  }, []);

  useEffect(() => {
    if (pelanggan.length > 0) {
      setValue('kode_pelanggan', pelanggan[0].kode_pelanggan || '');
      setValue('jenis_akun', pelanggan[0].jenis_akun || '');
      setValue('cabang_asal', pelanggan[0].cabang_asal || '');
      setValue('kantor_asal', pelanggan[0].kantor_asal || '');
      setValue('kode_resi', pelanggan[0].kode_resi || '');
      setValue('jenis_industri', pelanggan[0].jenis_industri || '');
      setValue('vendor_integration', pelanggan[0].vendor_integration || '');
      setValue(
        'pilihan_transaksi',
        pelanggan[0].pilihan_transaksi.split(',') || ''
      );
      setValue('pembulatan_kilo', pelanggan[0].pembulatan_kilo || '');
      setValue('diskon_pelanggan', pelanggan[0].diskon_pelanggan || '');
      setValue('logo_perusahaan', pelanggan[0].logo_perusahaan || '');
      setValue('single_akun', pelanggan[0].single_akun || '');
      setValue('pelanggan_nasional', pelanggan[0].pelanggan_nasional || '');
      setValue('kode_grup', pelanggan[0].kode_grup || '');
      setValue('distribusi_stok_awb', pelanggan[0].distribusi_stok_awb || '');
      setValue('masa_efektif', pelanggan[0].masa_efektif || '');
      setValue('masa_berakhir', pelanggan[0].masa_berakhir || '');
      setValue('nama_pelanggan', pelanggan[0].nama_pelanggan || '');
      setValue('divisi', pelanggan[0].divisi || '');
      setValue('departemen', pelanggan[0].departemen || '');
      setValue('alamat', pelanggan[0].alamat || '');
      setValue('kota', pelanggan[0].kota || '');
      setValue('kode_pos', pelanggan[0].kode_pos || '');
      setValue('telepon', pelanggan[0].telepon || '');
      setValue('no_fax', pelanggan[0].no_fax || '');
      setValue('email', pelanggan[0].email || '');
      setValue('website', pelanggan[0].website || '');
      setValue('npwp', pelanggan[0].npwp || '');
      setValue('nppkp', pelanggan[0].nppkp || '');
      setValue('nama_kontak', pelanggan[0].nama_kontak || '');
      setValue('pekerjaan_kontak', pelanggan[0].pekerjaan_kontak || '');
      setValue('telepon_kontak', pelanggan[0].telepon_kontak || '');
      setValue('email_kontak', pelanggan[0].email_kontak || '');
      setValue('periode_penagihan', pelanggan[0].periode_penagihan || '');
      setValue('top', pelanggan[0].top || '');
      setValue(
        'penagihan_dengan_pod_balik',
        pelanggan[0].penagihan_dengan_pod_balik || ''
      );
      setValue(
        'penagihan_dengan_email',
        pelanggan[0].penagihan_dengan_email || ''
      );
      setValue(
        'jenis_pembayaran',
        pelanggan[0].jenis_pembayaran.split(',') || ''
      );
      setValue('no_virtual_account', pelanggan[0].no_virtual_account || '');
      setValue('hari_kerja', pelanggan[0].hari_kerja || '');
      setValue('jam_kerja', pelanggan[0].jam_kerja || '');
      setValue('penjemputan', pelanggan[0].penjemputan || '');
      setValue('jadwal_penjemputan', pelanggan[0].jadwal_penjemputan || '');
      setValue('waktu_penjemputan', pelanggan[0].waktu_penjemputan || '');
      setValue('sales', pelanggan[0].sales || '');
      setValue('periode_awal', pelanggan[0].periode_awal || '');
      setValue('periode_akhir', pelanggan[0].periode_akhir || '');
      setValue('target_nilai', pelanggan[0].target_nilai || '');
      setValue('telepon_sales', pelanggan[0].telepon_sales || '');
      setValue('email_sales', pelanggan[0].email_sales || '');
      setValue('dibuat_oleh', pelanggan[0].dibuat_oleh || '');
      setValue('diajukan_oleh', pelanggan[0].diajukan_oleh || '');
      setValue('tanggal_dibuat', pelanggan[0].tanggal_dibuat || '');
    }
  }, [pelanggan]);

  return (
    <Page title="Ubah Pelanggan | Sarana Mulya">
      <Container maxWidth="xl">
        <Box sx={{ pb: 5 }}>
          <Typography variant="h4">Ubah Pelanggan</Typography>
        </Box>
        <Card sx={{ p: 5 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  ...
                  <InputDropdown
                    ...
                    options={periodePenagihanOptions}
                    control={control}
                  />
                </Grid>
                <Grid item xs={12}>
